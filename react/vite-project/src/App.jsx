...
import CareerForm from "./components/careerform"; // ✅ Added from 2nd code
...
// ✅ LogoOnly Component (for minimal header on form pages)
function LogoOnly() {
  return (
...
function AppWrapper() {
  ...
  // ✅ Conditional Footer & Chatbot
  const showFooterChatbot =
    location.pathname !== "/login" &&
    location.pathname !== "/form" &&
    location.pathname !== "/career-form";
        ...
        <Route
          ...
          element={
            <>
              <Hero user={user} />
              <Features />
              {showFooterChatbot && <Chatbot />}
              {showFooterChatbot && <Footer />}
            </>
          }
        />
